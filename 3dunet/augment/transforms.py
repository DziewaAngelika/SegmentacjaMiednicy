    ...
    def __call__(self, m):
        if self.random_state.uniform() < self.execution_probability:
            if(m.ndim == 3):
                ...
                dz = gaussian_filter(self.random_state.randn(*m[0,:,:,:].shape), self.sigma, mode="constant", cval=0) * self.alpha
                dy = gaussian_filter(self.random_state.randn(*m[0,:,:,:].shape), self.sigma, mode="constant", cval=0) * self.alpha
                dx = gaussian_filter(self.random_state.randn(*m[0,:,:,:].shape), self.sigma, mode="constant", cval=0) * self.alpha
